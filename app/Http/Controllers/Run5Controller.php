...
class Run5Controller extends Controller
{
    public function img_data(Request $rq){
        try{
        $image= $rq->input('img_data');
        $image = trim(str_replace('data:image/png;base64,',"", $image));
        $filename="img-".parent::get_s("rand").".png";
        //$image->move(public_path('assets/img/graphe'),$filename);
        //$image=base64_decode($image);
        
        file_put_contents(public_path('assets/img/graphe/').$filename,base64_decode($image));
        //Storage::disk('public')->put($filename,base64_decode($image));
        return asset("assets/img/graphe/".$filename);
        }catch(Throwable $e){
            return $e->getMessage();
        }
    }
    ...
    public function send_ballon1(Request $rq){
        try{
            $validation = Validator::make($rq->all(), [
                'vol' => 'required','lon' =>'required','dia' =>'required',
                'epa'=> 'required','con' => 'required',
            ]);
            if(!$validation->passes())
                return "Saisir les informations";
            parent::put_s('vol1',$rq->vol);
            parent::put_s('lon1',$rq->lon);
            parent::put_s('dia1',$rq->dia);
            parent::put_s('epa1',$rq->epa);
            parent::put_s('con1',$rq->con);           
            return "L'enregistrement a été effectué avec succès"; 
        }catch(Throwable $e){
            return $e->getMessage();
        }
    }
    public function send_ballon2(Request $rq){
        try{
            $validation = Validator::make($rq->all(), [
                'vol' => 'required','lon' =>'required','dia' =>'required',
                'epa'=> 'required','con' => 'required',
            ...
            if($rq->flagPB=="on"){
                ...
                parent::put_s('prixbal2',$rq->prixbal);
            }
            if($rq->sysAppoint=="2"){
                $validation = Validator::make($rq->all(), [
                ...
                parent::put_s('pci2',$rq->pci);
                parent::put_s('prixKWH2',$rq->prixKWH);
            }else if($rq->sysAppoint=="0"){
                return "Veuillez sélectionner un système d'appoint.";
            }
            parent::put_s("sysAppoint2",$rq->sysAppoint);
            parent::put_s('vol2',$rq->vol);
            parent::put_s('lon2',$rq->lon);
            parent::put_s('dia2',$rq->dia);
            parent::put_s('epa2',$rq->epa);
            parent::put_s('con2',$rq->con);        
            $tab =array();
            for($i=1; $i<=12 ; $i++){
                $tab[]=$rq->input('tab'.$i);
            }
            parent::put_s('tab_mois2',$tab);
            return "L'enregistrement a été effectué avec succès"; 
        }catch(Throwable $e){
            return $e->getMessage();
        }
    ...
    public function send_pompe3(Request $rq){
        try{
            $validation = Validator::make($rq->all(), [
                'puiMax' => 'required','debitNom'=>'required',
            ]);
            if(!$validation->passes())
                return "Saisir les informations";
            parent::put_s('puiMax3',$rq->puiMax);
            parent::put_s('debitNom3',$rq->debitNom);
            return "L'enregistrement a été effectué avec succès"; 
        }catch(Throwable $e){
            return $e->getMessage();
        }
    ...
    public function send_tuyau3(Request $rq){
        try{
            $validation = Validator::make($rq->all(), [
                'diaInt' => 'required','diaExt'=>'required',
                'longAll'=>'required','longRet'=>'required',
            ...
            if(!$validation->passes())
                return "Saisir les informations";
            parent::put_s('diaInt3',$rq->diaInt);
            parent::put_s('diaExt3',$rq->diaExt);
            parent::put_s('longAll3',$rq->longAll);
            parent::put_s('longRet3',$rq->longRet);
            parent::put_s('isolEpa3',$rq->isolEpa);
            parent::put_s('isolCon3',$rq->isolCon);
            return "L'enregistrement a été effectué avec succès"; 
        }catch(Throwable $e){
            return $e->getMessage();
        }
    ...
    public function send_chauf(Request $rq){
        try{
            $validation = Validator::make($rq->all(), [
                'temChauf'=>'required','coefChauf'=>'required',
            ]);
            if(!$validation->passes())
                return "Saisir les informations";
            if($rq->volChauf=="")
                parent::put_s('volChauf',"0");
            else
                parent::put_s('volChauf',$rq->volChauf);
            parent::put_s('temChauf',$rq->temChauf);
            parent::put_s('coefChauf',$rq->coefChauf);
            return "L'enregistrement a été effectué avec succès"; 
        }catch(Throwable $e){
            return $e->getMessage();
        }
    ...
    public function exportPdf(Request $rq){
        ...
        //return view("pdf5",$data);
        $pdf = PDF::setOptions([
        'dpi' => 150,
        'defaultFont' => 'sans-serif',
        'enable-javascript' => true,
        'images' => true])->loadView('pdf5',$data);
    ...
    public function play(){
        try{
            ...
            parent::put_s("stat13",$this->tcbPP1());
            parent::put_s("stat14",$this->tcbPP2());
            parent::put_s("stat15",$this->pethe2());
            parent::put_s("stat16",$this->pethe1());
            parent::put_s("stat17",$this->petu());
            parent::put_s("stat18",$this->ecen1());
            parent::put_s("stat19",$this->conpom());
            parent::put_s("stat20",[$this->TRI(),$this->gainC()]);           
            ...
            "crefr"=>parent::get_s("stat8"),"pethe2"=>parent::get_s("stat15"),
            "pethe2S"=>array_sum(parent::get_s("stat15")),"pethe1"=>parent::get_s("stat16"),
            "pethe1S"=>array_sum(parent::get_s("stat16")),
            "petu"=>parent::get_s("stat17"),"betheP"=>$this->betheP(),
            "bechauf"=>$this->bechauf(),
            "petuS"=>array_sum(parent::get_s("stat17")),"ecen1"=>parent::get_s("stat18"),
            "ecen1S"=>array_sum(parent::get_s("stat18")),"conpom"=>parent::get_s("stat19"),
            "conpomS"=>array_sum(parent::get_s("stat19")),"gainF"=>$this->gainF(),
            "gainA"=>array_sum($this->gainF()),"gainC"=>parent::get_s("stat20")[1],
            "tri"=>parent::get_s("stat20")[0],"co2"=>$this->co2(),"besoinEau"=>parent::get_s("besoinTotal"),
            "ville"=>parent::get_s("ville"),"zone"=>parent::get_s("zone"),"ef"=>parent::get_s("ef"),
            "vol1"=>parent::get_s("vol1"),"vol2"=>parent::get_s("vol2"),"dhw"=>parent::get_s("dhw"),
    ...
    private function betheP(){//B𝒆𝒔𝒐𝒊𝒏 𝒕𝒉𝒆𝒓𝒎𝒊𝒒𝒖𝒆 
        try{
            if(!parent::has_s('zone'))
                throw new \Exception("Veuillez sélectionner la zone");
            if(!parent::has_s('dhw'))
            ...
            $nbr=$this->StoF(parent::get_s("nbrPerson"));
            $res=array();
            for($i=0;$i<12;$i++){
    ...
    private function bechauf(){
        try{
            if(!parent::has_s('volChauf'))
                throw new \Exception("Veuillez entrer le volume des pièces à chauffer");
            if(!parent::has_s('temChauf'))
                throw new \Exception("Veuillez entrer la température de chauffage");
            if(!parent::has_s('coefChauf'))
                throw new \Exception("Veuillez entrer le coefficient de déperdition globale");
            $v=$this->StoF(parent::get_s("volChauf"));
            $t=$this->StoF(parent::get_s("temChauf"));
            $c=$this->StoF(parent::get_s("coefChauf"));
            $ta=parent::get_s("tm");
            $mois=[31,29,31,30,31,30,31,31,30,31,30,31];
            $res=array();
            for($i=0;$i<count($ta);$i++){
                if($t>$ta[$i])
                    $res[]=round($v*$c*($t-$ta[$i])*24*$mois[$i]*pow(10,-3),2);
                else 
                    $res[]=0;
            }
            return $res;
    ...
    private function bethe(){
        try{
            $tch=$this->bechauf();
            $be=$this->betheP();
            $res=array();
            for($i=0; $i<12; $i++){
                $res[]=round($tch[$i]+$be[$i],2);
            }
            return $res;
        }catch(Throwable $e){
            throw new \Exception($e->getMessage());
    ...
    public function qaux(){
        try{
            ...
            $qucollM=$this->qucollM();
            $bech=$this->bechauf();
            if(!parent::has_s("tab_mois2"))
                throw new \Exception("veuillez entrer la liste des activations de résistance");
            $x=parent::get_s("tab_mois2");
            $res=array();
            for($i=0; $i<12; $i++){
                if($bethe[$i]>$qucollM[$i])
                    $res[]=round(($bethe[$i]-$qucollM[$i])*$this->StoF($x[$i])+0.2*$bethe[$i],2);
                else
                    $res[]=0;
            }
            return $res;
    ...
    private function caux(){
        try{
            ...
            $res=array();
            /*
            if(parent::get_s("sysAppoint2")=="1"){//electrique
                for($i=0;$i<12; $i++){
            ...
            }else*/ if(parent::get_s("sysAppoint2")=="1"){//gaz
                for($i=0;$i<12; $i++){
                    $res[]=round($qaux[$i]*0.82,2);
                }
            }else if(parent::get_s("sysAppoint2")=="2"){//autre
                if(!parent::has_s("prixKWH2"))
                    throw new \Exception("Veuillez entrer le prix du Kwh électricité pour d'autre pays ($) ");
                for($i=0;$i<12; $i++){
                    $res[]=round($qaux[$i]*$this->StoF(parent::get_s("prixKWH2")),2);
                }
            }
            return $res;
        }catch(Throwable $e){
            ...
            if(parent::get_s("sys_r")=="2"){//electrisité
                ...
                for($i=0; $i<12; $i++){
                    if(parent::get_s("autre_v")){
                        $res[]=round($bethe[$i]*$this->StoF(parent::get_s("kwat")),1);
                    }else{
                        if($bethe[$i]<100) $res[]=$bethe[$i]*0.9;
                        if($bethe[$i]>100 && $bethe[$i]<=150) $res[]=round($bethe[$i]*1.07,1);
                        if($bethe[$i]>150 && $bethe[$i]<=210) $res[]=round($bethe[$i]*1.07,1);
                        if($bethe[$i]>210 && $bethe[$i]<=310) $res[]=round($bethe[$i]*1.16,1);
                        if($bethe[$i]>310 && $bethe[$i]<=510) $res[]=round($bethe[$i]*1.38,1);
                        if($bethe[$i]>510) $res[]=round($bethe[$i]*1.59,1);
                    }                
                }
    ...
    private function vp(){
        try{
            ...
            if(!parent::has_s("besoinTotal"))
                throw new \Exception("Veuillez entrer le besoin totale.");
            $be=$this->StoF(parent::get_s("besoinTotal"));
            $tab_consom=explode(",",parent::get_s("tab_consom"));
            $res=array();
            for($i=0; $i<count($tab_consom);$i++){
                $res[]=($be*$tab_consom[$i])/100;
            }
            return $res;
        }catch(Throwable $e){
            throw new \Exception($e->getMessage());
    ...
    private function tcbP1(){
        try{
            if(!parent::has_s("vol1"))
                throw new \Exception("Veuillez entrer le  volume du ballon primaire.");
            if(!parent::has_s("epa1"))
                throw new \Exception("Veuillez entrer l'epaisseur isolant du ballon primaire.");
            if(!parent::has_s("dia1"))
                throw new \Exception("Veuillez entrer le diamètre du ballon primaire.");
            if(!parent::has_s("lon1"))
                throw new \Exception("Veuillez entrer la longeur du ballon primaire.");
            if(!parent::has_s("con1"))
                throw new \Exception("Veuillez entrer la conductivité thermique du ballon primaire.");
            $vol=$this->StoF(parent::get_s("vol1"));
            $epa=$this->StoF(parent::get_s("epa1"));
            $dia=$this->StoF(parent::get_s("dia1"));
            $lon=$this->StoF(parent::get_s("lon1"));
            $con=$this->StoF(parent::get_s("con1"));
            $tsc=$this->tsc();
            $ef=parent::get_s("ef");
            $ta=parent::get_s("th");
            $res=array();
            for($i=0; $i<count($tsc);$i++){
                $dt=0;
                if($tsc[$i]>$ta[$i]){
                    $dt=(($tsc[$i]-$ta[$i])*(3.14*$dia*$lon+3.14*pow($dia,2)/4)*$con*3.6)/($epa*$vol*4.19);
                }   
                $res[]=$tsc[$i]-$dt;      
            }
            return $res;
    ...
    private function tcbPP1(){
        try{
            $ef=parent::get_s("ef");
            $tcbp=$this->tcbP1();
            $vp=$this->vp();
            $v=$this->StoF(parent::get_s("vol1"));
            $res=array();
    ...
    private function tcbP2(){
        try{
            if(!parent::has_s("vol2"))
                throw new \Exception("Veuillez entrer le  volume du ballon secondaire.");
            if(!parent::has_s("epa2"))
                throw new \Exception("Veuillez entrer l'epaisseur isolant du ballon secondaire.");
            if(!parent::has_s("dia2"))
                throw new \Exception("Veuillez entrer le diamètre du ballon secondaire.");
            if(!parent::has_s("lon2"))
                throw new \Exception("Veuillez entrer la longeur du ballon secondaire.");
            if(!parent::has_s("con2"))
                throw new \Exception("Veuillez entrer la conductivité thermique du ballon secondaire.");
            $vol=$this->StoF(parent::get_s("vol2"));
            $epa=$this->StoF(parent::get_s("epa2"));
            $dia=$this->StoF(parent::get_s("dia2"));
            $lon=$this->StoF(parent::get_s("lon2"));
            $con=$this->StoF(parent::get_s("con2"));
            $tcbpp=$this->tcbPP1();
            $ta=parent::get_s("th");
            $res=array();
            for($i=0; $i<count($tcbpp);$i++){
                $dt=0;
                if($tcbpp[$i]>$ta[$i]){
                    $dt=(($tcbpp[$i]-$ta[$i])*(3.14*$dia*$lon+3.14*pow($dia,2)/4)*$con*3.6)/($epa*$vol*4.19);
                }   
                $res[]=$tcbpp[$i]-$dt;      
            }
            return $res;
    ...
    private function tcbPP2(){
        try{
            $ef=parent::get_s("ef");
            $tcbp2=$this->tcbP2();
            $tcbpp1=$this->tcbPP1();
            $vp=$this->vp();
            $v=$this->StoF(parent::get_s("vol2"));
            $res=array();
            for($i=0;$i<count($tcbp2);$i++){
                $res[]=(($v-$vp[$i%count($vp)])*$tcbp2[$i]+$vp[$i%count($vp)]*$tcbpp1[$i%count($tcbpp1)])/$v;
            }
            return $res;
        }catch(Throwable $e){
            throw new \Exception($e->getMessage());
