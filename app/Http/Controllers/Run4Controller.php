...
class Run4Controller extends Controller
{
    ...
    // /!\Important j'ai changé les noms des ballons
    public function send_ballon1(Request $rq){
        try{
            ...
            if($rq->flagPB=="on"){
                ...
                if(!$validation->passes())
                    return "Veuillez saisir le prix du Kwh électricité d'un autre pays.";
                parent::put_s('prixbal1',$rq->prixbal);
            }
            if($rq->sysAppoint=="3"){
                $validation = Validator::make($rq->all(), [
    ...
    public function exportPdf(Request $rq){
        ...
        'images' => true])->loadView('pdf4',$data);
    ...
    public function play(){
        try{
            ...
            "fsM"=>array_sum(parent::get_s("stat5"))/12,"qaux"=>parent::get_s("stat6"),
            ...
            "pethe1S"=>array_sum(parent::get_s("stat16")),"petu"=>parent::get_s("stat17"),
    ...
    private function caux(){
        try{
            ...
            }else if(parent::get_s("sysAppoint2")=="2"){//gaz
                for($i=0;$i<12; $i++){
                    $res[]=$qaux[$i]*0.82;
                }
            }else if(parent::get_s("sysAppoint2")=="3"){//autre
                if(!parent::has_s("prixKWH2"))
                    throw new \Exception("Veuillez entrer le prix du Kwh électricité pour d'autre pays ($) ");
                for($i=0;$i<12; $i++){
                    $res[]=$qaux[$i]*$this->StoF(parent::get_s("prixKWH2"));
                }
            }
            return $res;
        }catch(Throwable $e){
            ...
            if(parent::get_s("sys_r")=="2"){//electrisité
                ...
                for($i=0; $i<12; $i++){
                    if(parent::get_s("autre_v")){
                        ...
                        if($bethe[$i]<100) $res[]=round($bethe[$i]*0.9,1);
