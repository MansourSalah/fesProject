...
class Run2Controller extends Controller
{
    ...
    public function send_ballon(Request $rq){
        try{
            ...
            if($rq->sysAppoint=="3"){
                ...
                return "Veuillez sélectionner le type de système d’appoint";
            }
    ...
    public function send_pompe(Request $rq){
        try{
            $validation = Validator::make($rq->all(), [
            ...
            if(!$validation->passes())
                return "Saisir les informations";
            parent::put_s('puiMax',$rq->puiMax);
            parent::put_s('debitNom',$rq->debitNom);
            return "L'enregistrement a été effectué avec succès"; 
        }catch(Throwable $e){
            return $e->getMessage();
        }
    ...
    public function send_dolar(Request $rq){
        try{
            ...
            parent::put_s('inve',$rq->inve);//investissement
            if($rq->unite=="dhm")
    ...
    public function exportPdf(Request $rq){
        ...
        'images' => true])->loadView('pdf2',$data);
    ...
    public function play(){
        try{
            ...
            "tri"=>parent::get_s("stat18")[0],"co2"=>$this->co2(),
            "ville"=>parent::get_s("ville"),"zone"=>parent::get_s("zone"),"ef"=>parent::get_s("ef"),
            "besoinEau"=>parent::get_s("besoinTotal"),"vol"=>parent::get_s("vol"),"dhw"=>parent::get_s("dhw"),
    ...
    private function conpom(){
        try{
            if(!parent::has_s('puiMax'))
                throw new \Exception("Veuillez entrer la puissance de la pompe.");
            $p=$this->StoF(parent::get_s("puiMax"));
            $g=parent::get_s("rh");
            $ta=parent::get_s("th");
            $res=array();
            $z=0;
