...
$(document).ready(function(){
  ...
  .done(function(data){
      ...
      for(i=0; i<grph.length;i++){
        if(grph[i][0]==1){//bethe
        ...
        }else if(grph[i][0]==7){//pethe
          tab_g=getY(grph[i][1],0,12);labels=getX(1,12);
          renderChart(tab_g,labels,"grph"+(i+1),"Pertes Thermiques (KWh)",colors[i%8]);
        }else if(grph[i][0]==8){//ecoen
          tab_g=getY(grph[i][1],0,12);labels=getX(1,12);
          renderChart(tab_g,labels,"grph"+(i+1),"Economie d’Energie",colors[i%8]);
        }else if(grph[i][0]==9){//crefr
          var crfr=grph[i][1];
        ...
        }else if(grph[i][0]==14){//TSC
          tab_g=getY(grph[i][1],data[3]-1,data[4]-1);labels=getX(data[3],data[4]);
          renderChart(tab_g,labels,"grph"+(i+1),"Température de sortie du capteur",colors[i%8]);
        }else if(grph[i][0]==15){//TSCpp
          tab_g=getY(grph[i][1],data[3]-1,data[4]-1);labels=getX(data[3],data[4]);
          renderChart(tab_g,labels,"grph"+(i+1),"Température du ballon",colors[i%8]);
        }else if(grph[i][0]==16){//gain cumulé
          ...
          //renderChart(tab_g,labels,"grph"+(i+1),"Gain cumulé",colors[i%8]);
        } 
      }
  });
});
